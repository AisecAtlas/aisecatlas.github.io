...
CISA Zero Trust Maturity Model v2를 따른 매트릭스다.

가로축은 성숙도 단계, 세로축은 보안 영역(Pillar)이다. 각 셀은 해당 영역과 단계를 대표하는 솔루션 카테고리다.
...
> 단계는 1에서 4로 가는 일직선이 아니다. 영역마다 다른 단계에 있을 수 있고, 조직 전체가 균등할 필요도 없다.

## 횡단 기능

특정 영역이 아니라 전 영역에 걸쳐 작동하는 기능이다.
...
- 영역별 깊이 있는 가이드는 [영역별 가이드](/pillars/)에서 본다.
- 점수가 어떻게 매겨지는지는 [평가 방법](/evaluation/scoring-rubric/)에서 확인한다.
- 영역 정의의 출처는 [CISA ZTMM v2 요약](/framework/cisa-ztmm-v2/)에서 본다.
