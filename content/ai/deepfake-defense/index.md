...
> **이 페이지의 결론**: [AI를 지키는 일](/ai/defend-ai/)이 우리가 쓰는 AI 시스템을 보호하는 것이라면, 이 페이지는 반대편입니다. 공격자가 AI를 무기로 쓰는 경우, 즉 딥페이크 영상과 음성 복제로 사람을 속이는 사기를 다룹니다. 기술 탐지와 프로세스 통제를 함께 써야 막을 수 있습니다.
...
탐지 기술만으로는 부족합니다. 프로세스 통제를 함께 설계해야 합니다.
...
**금융권 비대면 실명확인.** 딥페이크 얼굴인식 우회는 비대면 계좌 개설과 대출의 KYC를 위협합니다. 라이브니스 위에 딥페이크 탐지 계층을 추가하는 방안을 검토하세요.
...
본 평가는 공개 자료를 근거로 합니다. 피해 통계는 보도와 분석 기관 추정치이며 출처에 따라 다릅니다. 도입 전에 자체 환경에서 탐지 정확도를 검증하세요.
