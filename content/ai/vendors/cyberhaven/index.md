...
Cyberhaven은 직원 AI 사용행위 보호(②)를 **데이터 리니지** 각도로 푸는 스타트업입니다. 질문이 다릅니다. "프롬프트에 무엇을 넣었나"(내용 검사)가 아니라 **"어떤 데이터가 어디서 와서 AI로 갔나"**(출처와 맥락)입니다. 엔드포인트 에이전트가 데이터의 생성부터 모든 이동을 기록하고, 출처로 분류해 AI로 가는 흐름을 통제합니다. Linea AI가 리니지 모델로 위험을 탐지하고 조사합니다.
