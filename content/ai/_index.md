...
**클라우드 LLM 사용.** 금감원 클라우드 보안성 심의는 LLM API 호출 흐름도 검토 대상이 될 수 있다. 데이터 리전과 학습 사용 동의를 확인한다.

**망분리 환경.** 외부 LLM API를 직접 호출할 수 없는 조직은 사내 LLM과 자체 가드레일을 구성한다.

한국 규제 변수는 [표준과 규제](/framework/)가 정리한다.
