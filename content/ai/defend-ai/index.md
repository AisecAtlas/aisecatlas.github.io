---
title: "AI를 지키는 일"
description: "LLM 애플리케이션과 AI 에이전트를 공격으로부터 보호한다. OWASP 기준과 주요 카테고리 정리."
date: 2026-05-30
lastmod: 2026-05-30
...
> **이 페이지의 결론**: 2026년 현재 AI 보안의 1순위 위협은 여전히 **Prompt Injection**이며, 특히 **간접(Indirect) Injection**이 에이전트 시스템에서 핵심 공격면이 되었다. OWASP는 LLM과 Agentic 시스템을 위해 **별도의 Top 10**을 유지하고, 시장에는 4가지 카테고리의 방어 도구가 형성됐다.
...
기존 보안 영역과 겹치는 부분도 있지만, **공격면이 자연어**라는 점이 결정적으로 다르다. 정규표현식과 시그니처로 막을 수 없다.

## OWASP 표준 두 가지

### OWASP LLM Top 10 v2.0 (2025)

단일 호출 LLM 애플리케이션을 위한 표준이다. **2025년에 v2.0으로 개정**되면서 RAG 파이프라인과 멀티 모델 아키텍처의 위협을 반영했다.
...
자율 행동을 하는 에이전트 시스템 전용이다. **100명 이상의 산업 전문가가 검토**한 별도 프레임워크로, 2026년에 발표됐다.
...
> **왜 별도 표준이 필요한가**: 에이전트는 단일 호출 LLM과 달리 **추론, 메모리, 도구, 다단계 실행**이 결합된다. 프롬프트 단계 방어만으로 막을 수 없는 새로운 취약점 분류가 생긴다.

에이전트 신원(NHI)과 MCP 위협 대응은 [Agentic AI 보안과 MCP](/ai/agentic-security/)에서 다룬다.

## Prompt Injection이 여전히 1순위인 이유

2025-2026년의 변화 세 가지가 결합됐다.

**Multi-turn jailbreak**가 frontier model의 주된 공격 패턴이 되었다. 한 번의 프롬프트가 아니라 여러 턴에 걸쳐 점진적으로 가드레일을 무너뜨린다.

**Multimodal injection**이 성숙했다. 이미지, QR 코드, 스테가노그래픽 페이로드에 명령을 숨겨 전달한다. 텍스트 필터만으로는 잡히지 않는다.

**MCP 서버 악용**이 새 공격면으로 떠올랐다. **Tool Poisoning, Credential Theft**가 에이전트 도입과 함께 등장했다. Lakera의 분석에 따르면 **간접 공격이 직접 공격보다 적은 시도로 성공**한다. 외부 콘텐츠를 가져오는 흐름 자체가 1차 위험 벡터다.

## 시장 카테고리

2026년 현재 AI 방어 도구는 네 가지 카테고리로 나뉜다.

### 1. AI 게이트웨이 / Runtime 가드레일

LLM 호출 앞단에서 입출력을 검사하고 차단한다. 같은 척도의 심층 비교는 [LLM 게이트웨이와 런타임 가드레일 비교](/ai/llm-gateway/)에서 다룬다.
...
배포 전에 LLM 앱과 에이전트를 자동 공격해 취약점을 찾는다. 같은 척도의 심층 비교와 MITRE ATLAS 매핑은 [AI 레드티밍 비교](/ai/ai-red-teaming/)에서 다룬다.
...
거버넌스 차원에서 AI 사용 인벤토리, 데이터 흐름, 정책 적합성을 관리한다. 규제 프레임워크(EU AI Act, NIST AI RMF, ISO 42001) 매핑과 솔루션 비교는 [AI 거버넌스와 TRiSM](/ai/ai-governance/)에서 다룬다.
...
학습 단계와 모델 레지스트리 보안이다. 악성 모델, pickle 공격, 모델 서명과 ML-BOM 심층 내용은 [AI 모델 공급망 보안](/ai/model-supply-chain/)에서 다룬다.
...
자산 발견과 태세 관리(AI-SPM)는 [AI-SPM 솔루션](/ai/ai-spm/)에서 다룬다.

## 도입 의사결정 가이드

### 어디서부터 시작하나

조직의 AI 도입 단계별로 우선순위가 다르다.
...
**데이터 처리.** Prompt에 개인정보가 섞이는 흐름은 **개인정보보호법 위반 소지**가 있다. 입출력 필터에 PII 마스킹이 필수다.

**클라우드 LLM 호출.** 금감원의 클라우드 보안성 심의가 LLM 흐름까지 검토 범위에 들어간다. **데이터 리전, 학습 사용 동의, 로그 보존 정책**을 확인한다.

**망분리 환경.** 외부 LLM API를 직접 호출할 수 없는 조직은 자체 호스팅 LLM과 자체 가드레일 조합이 현실적이다. 오픈소스 도구(NeMo Guardrails 등)와 사내 OPA 정책의 결합 패턴이 자주 쓰인다.

**공공 부문.** CSAP 인증을 보유한 클라우드에서 운영되는 LLM 서비스만 사용 가능한 경우가 많다. Microsoft Azure OpenAI Service의 한국 리전 활용 사례가 는다.
