...
- **③ AI로 보안을 강화한다** — AI가 SOC 운영을 가속합니다. AI를 보호하는 일이 아니라 보안에 AI를 쓰는 일이라 축을 분리합니다.
...
열은 세 기능, 행은 벤더입니다. 빈 칸은 해당 벤더가 다루지 않는 기능입니다.
...
①②③을 한 플랫폼에서 운영하는 벤더는 기존 플랫폼(SASE, 엔드포인트, M365, 네트워크, CNAPP) 위에 AI 보안을 얹습니다. 빈 자리를 인수로 채운 경우가 많습니다(Lakera는 Check Point, Protect AI는 Palo Alto, Robust Intelligence는 Cisco, Prompt Security는 SentinelOne으로). 단일 통제 지점에 특화한 벤더는 인접 기능으로 범위를 넓혀 갑니다.

## 통제 지점 (PEP 위치)

도입 전제를 가르는 건 통제 지점의 위치입니다. 같은 ② 직원 AI 사용 통제라도 네트워크에서 막으려면 SASE가 있어야 하고, 단말에서 막으려면 단말마다 소프트웨어를 배포해야 합니다.
