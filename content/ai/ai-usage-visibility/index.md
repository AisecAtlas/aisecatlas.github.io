...
직원이 ChatGPT, Claude, Gemini, Copilot에 무엇을 입력하는지는 전부 **TLS로 암호화**됩니다. 네트워크 중간에서 그냥 보는 것으로는 **어떤 AI 서비스를 썼는지(도메인)**까지만 알 수 있고 **프롬프트 내용은 못 봅니다**. 내용을 보려면 셋 중 하나입니다. ① TLS를 끊고 재서명하는 **인라인 프록시**(루트 CA 설치 필요, 인증서 핀닝 앱은 우회=사각지대), ② 앱이 평문을 직접 보내도록 만든 **AI 게이트웨이**(우리가 통제하는 사내 앱만), ③ 암호화 전에 읽는 **엔드포인트/브라우저 캡처**(핀닝과 무관, 직원 브라우저 사용까지 포착). 미승인 Shadow AI 발견은 ②가 닿지 않으므로 ①과 ③의 조합이 현실적입니다.

## 네트워크 가시화의 한계

문제는 단순합니다. 생성형 AI 앱은 전부 **TLS 1.2/1.3**으로 암호화합니다. 회선 중간의 수동 캡처는 암호문과 메타데이터(목적지 IP, SNI 호스트명, DNS 질의, 패킷 크기)만 봅니다. **사용자가 입력한 프롬프트 텍스트**를 읽으려면 암호화를 끊거나, 암호화 전을 읽거나, 평문을 받는 정당한 종단이 되어야 합니다.

여기에 사용량 압박이 겹쳤습니다. Cyberhaven은 기업 데이터가 AI 도구로 흘러간 양이 전년 대비 **485% 증가**했다고 분석했습니다([Cyberhaven](https://www.cyberhaven.com/product/linea)). 직원은 개인 계정으로 무단 AI에 사내 자료를 붙여넣고, 보안팀은 그 사실 자체를 모릅니다. 이것이 **Shadow AI**입니다.

가시화는 두 질문으로 갈립니다.

- **무엇을 보려는가.** "어떤 AI를 쓰나"(메타데이터)면 DNS, SNI로 충분합니다. "무엇을 입력했나"(콘텐츠)면 복호화 또는 엔드포인트가 필요합니다.
- **어디를 보려는가.** 우리가 만든 **승인 AI**(사내 LLM 앱, Copilot, Bedrock)는 클라우드 API나 게이트웨이로 봅니다. 직원이 쓰는 **미승인 AI**(웹 ChatGPT, DeepSeek)는 네트워크나 엔드포인트로 잡습니다. 승인 자산 인벤토리는 [AI-SPM](/ai/ai-spm/)이 담당합니다.

## 8가지 캡처 메커니즘

같은 "AI 가시화" 솔루션도 **어디서 무엇을 캡처하느냐**가 전혀 다릅니다. 이 차이가 시장을 가르는 핵심 필터입니다.
...
- **인라인 CASB.** 사용자와 SaaS 사이 실시간 경로에서 검사(§1과 같은 TLS 가로채기 원리). 프롬프트를 실시간으로 보고 막습니다.
- **API CASB.** SaaS 테넌트에 **제공자 API(OAuth)로 연결**, 경로 밖에서 정지 데이터, 구성, 공유, 권한을 스캔합니다. Cloudflare는 ChatGPT, Claude, Gemini를 "에이전트리스 API 통합으로 연결해 내부 데이터와 태세를 스캔"한다고 밝힙니다([Cloudflare — CASB AI](https://blog.cloudflare.com/casb-ai-integrations/)).
- **차이.** 인라인은 **실시간 프롬프트 차단**(복호화 의존, 오프네트워크 시 우회됨). API는 **사후/정지 데이터**(실시간 입력은 못 봄). 둘은 보완 관계입니다([TechTarget](https://www.techtarget.com/searchsecurity/tip/Choosing-between-proxy-vs-API-CASB-deployment-modes)).

### 4. SWG (SASE/SSE의 웹 egress 통제점)

URL/도메인 분류("생성형 AI" 카테고리), 허용/차단, 그리고 **TLS 인스펙션을 켜면** 복호화된 GenAI 트래픽에 인라인 DLP를 겁니다. Zscaler는 "전송된 실제 입력 프롬프트를 본다"를 "SSL 인스펙션이 있는 인라인 클라우드 프록시"로 제공한다고 명시합니다([Zscaler — AI Data Protection](https://www.zscaler.com/blogs/product-insights/new-ai-innovations-supercharge-data-protection)).
...
리눅스 커널에서 샌드박스 프로그램을 훅(kprobe, uprobe)에 붙입니다. 암호화 페이로드는 **유저스페이스 TLS 라이브러리 함수에 uprobe**(예: OpenSSL `SSL_read`/`SSL_write`)를 걸어 **암호화 직전/복호화 직후 평문**을 가로챕니다 — TLS를 끊지도 키를 공유하지도 않습니다([Pixie — eBPF TLS tracing](https://blog.px.dev/ebpf-tls-tracing-past-present-future/)).

- **한계.** **암복호화를 수행하는 그 노드 위**에서만, 동적 링크된 OpenSSL 같은 훅 가능한 심볼이 있을 때만 페이로드를 봅니다. 네트워크 엣지에서는 복호화 못 하고, 정적 링크(BoringSSL)나 자바 JSSE는 사각입니다([Coroot](https://coroot.com/blog/java-tls-instrumentation-with-ebpf/)). 직원 윈도우 노트북의 브라우저 프롬프트 캡처용이 아니라 **리눅스 서버의 AI 에이전트 관측**에 적합합니다.
...
> **핵심.** 네트워크의 어느 지점도 TLS 세션을 능동적으로 끊지 않으면 **암호문과 메타데이터만** 봅니다. 콘텐츠 가시성은 ① 사내 앱을 AI 게이트웨이로 보내거나 ② 엔드포인트/브라우저에서 암호화 전을 읽는 두 전략으로 수렴합니다. 직원 브라우저의 미승인 사용까지 잡는 가장 확실한 길은 **②**입니다.

## 솔루션 지형

벤더별 ZTA 아키텍처와 통제 지점(PEP) 위치 비교는 [벤더 프로필](/ai/vendors/)과 [AI 보안 지도](/ai/map/)가 정리합니다. 대표로 [Zscaler](/ai/vendors/zscaler/), [Harmonic](/ai/vendors/harmonic/), [WitnessAI](/ai/vendors/witnessai/), [Nightfall](/ai/vendors/nightfall/), [Cyberhaven](/ai/vendors/cyberhaven/)가 있습니다.

캡처 메커니즘으로 분류하면 시장이 깔끔하게 갈립니다.
...
> Palo Alto의 **AI Access Security**(직원의 외부 AI 사용 통제, SASE)와 **Prisma AIRS**(사내 구축 AI 보호, 런타임)는 다른 제품입니다. Microsoft도 **Defender for Cloud Apps**(미승인 발견)와 **Purview DSPM for AI**(승인 Copilot 데이터 태세)로 갈립니다. 후자(승인 자산)는 [AI-SPM](/ai/ai-spm/)이 다룹니다.

## 도입 의사결정 가이드

### 도입 우선순위

**1단계: 발견(미승인).** "직원이 어떤 AI를 쓰나"를 먼저 봅니다. 기존 SASE/SSE가 있으면 그 GenAI 카테고리(Zscaler, Netskope)로 즉시 시작. 없으면 DNS 계층(Umbrella)이 가장 싸고 빠른 출발점.

**2단계: 콘텐츠 가시성.** "무엇을 입력하나"가 필요하면 인라인 TLS 인스펙션을 켜되 **핀닝 앱 사각지대**를 인지합니다. 브라우저까지 확실히 잡으려면 브라우저 확장(Harmonic, LayerX)을 더합니다.

**3단계: 통제와 통합.** 발견 위에 DLP/정책(개인 계정 차단, PII 마스킹)을 겁니다. 승인 자산 인벤토리는 [AI-SPM](/ai/ai-spm/)으로, 사내 LLM 앱 런타임 방어는 [AI를 지키는 일](/ai/defend-ai/)로 잇습니다.
...
**망분리와 N²SF.** 업무망에서 외부 AI 자체가 차단된 환경이면 발견의 무게가 "DNS/프록시 차단 검증"으로 옮겨갑니다. [국가망보안체계(N²SF)](/korea/regulation/n2sf-mangbunri/) 전환으로 등급별 AI 허용이 생기면, 어느 등급 데이터가 어느 AI로 가는지 가시화가 핵심이 됩니다.

**개인정보보호법.** 미승인 AI에 PII가 붙여넣어지는 흐름은 위반 소지가 큽니다. 프롬프트 단위 DLP와 클립보드/붙여넣기 통제가 직접 통제점입니다.

**TLS 인스펙션의 법적, 운영 리스크.** 루트 CA 설치형 복호화는 직원 사생활 트래픽까지 봅니다. 검사 범위를 업무 AI 도메인으로 한정하고, 핀닝 앱 우회 목록을 사각지대로 문서화합니다.

**공공과 정부.** 클라우드 인라인 경유 자체가 [CSAP](/korea/regulation/csap-n2sf/)와 데이터 리전 검토 대상입니다. 단말 기반(브라우저 확장/엔드포인트) 캡처는 클라우드 경유 없이 적용 가능하나, 단말 텔레메트리의 처리, 보관 위치를 확인합니다.

**한국 시장 노트.** WitnessAI는 2026-01 라운드에 Samsung Ventures, Qualcomm Ventures가 참여했습니다(국내 진출 신호로 볼 여지, 공식 리전, 총판은 미발표). Zscaler, Netskope, Palo Alto, Microsoft는 국내 영업 거점이 있으나, **각 GenAI 모듈의 한국 데이터 리전 보장**은 공개 자료로 확인되지 않으니 직접 확인합니다.

## 관련 페이지

- [EDR 텔레메트리 기반 Shadow AI 헌팅](/ai/shadow-ai-hunting/) — 프록시가 닿지 않는 단말의 CLI, IDE 확장, MCP 서버 사용을 엔드포인트 텔레메트리로 헌팅
- [Zscaler](/ai/vendors/zscaler/) — 인라인 프록시 기반 SSE, AI Access Security
- [Harmonic Security](/ai/vendors/harmonic/) — 브라우저 확장 기반 Shadow AI 발견 특화
- [WitnessAI](/ai/vendors/witnessai/) — 단말 설치 없는 네트워크 인라인 가시화
- [AI 보안 솔루션 지도](/ai/map/) — 캡처 메커니즘과 통제 지점별 벤더 분포
- [벤더별 AI 보안](/ai/vendors/) — 벤더 프로필 전체
...
본 정리는 공개 자료를 근거로 합니다. 시장 변동이 빠른 영역입니다. 도입 전에 최신 공식 발표를 확인합니다. 벤더별 한국 데이터 리전과 핀닝 우회 범위는 직접 검증이 필요합니다.
