...
AI와 ML 시스템을 노리는 공격을 정리한 **적대적 위협 지식베이스**다. MITRE ATT&CK의 AI판으로, 전술과 기법을 매트릭스로 분류한다.
...
ATLAS는 다음 규모의 지식베이스다([Vectra 정리](https://www.vectra.ai/topics/mitre-atlas)).
...
전술은 공격자의 상위 목표(모델 회피, 모델 탈취, 데이터 포이즈닝 등)이고, 기법은 그 목표를 이루는 수단이다. 2025년에는 AI 에이전트를 겨냥한 **14개 기법**이 추가됐다. 프롬프트 인젝션과 메모리 조작 공격을 포함한다.
...
**AI 레드티밍.** 배포 전 공격 시나리오를 ATLAS 전술과 기법에 매핑하면 점검 범위가 명확해진다.
