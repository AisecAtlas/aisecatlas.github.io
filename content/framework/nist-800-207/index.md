...
- **마이크로세분화 (Micro-Segmentation).** 자원이나 자원 그룹을 게이트웨이가 보호하는 개별 세그먼트에 둡니다. NGFW, 지능형 스위치, 전용 게이트웨이가 PEP 역할을 합니다. 호스트 기반 에이전트나 엔드포인트 방화벽으로도 구현됩니다. 폭넓은 사용례에 적용되지만 신원 거버넌스 프로그램이 전제됩니다.
