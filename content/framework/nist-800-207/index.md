...
2023년 NIST는 클라우드 네이티브 환경의 신원 기반 세그멘테이션을 다루는 후속 [SP 800-207A](https://csrc.nist.gov/pubs/sp/800/207/a/final)를 별도 발간했습니다.
...
본 사이트는 솔루션 리뷰에서 이 7원칙을 표준 정합성 기준으로 사용합니다.

## 핵심 컴포넌트

Zero Trust는 다음 세 컴포넌트의 협업으로 작동합니다. 800-207은 의사결정을 담당하는 control plane(PE/PA)과 실제 트래픽이 흐르는 data plane(Subject↔PEP↔Resource)을 분리합니다.
...
NIST는 신뢰 알고리즘을 criteria-based 대 score-based, singular 대 contextual 두 축으로 분류합니다(원문 §3.3). 실무에서는 contextual + score-based 조합이 권장됩니다.

원문 §3.3은 입력으로 Access Request, Subject Database(주체 속성), Asset Database, Resource Policy Requirements, Threat Intelligence를 듭니다. 아래는 이를 실무 용어로 의역한 것입니다.
...
NIST는 원문 §3.1에서 네 가지 배포 패턴을 제시합니다.
...
과기정통부와 KISA의 한국형 제로트러스트 가이드라인(1.0/2.0)은 NIST 표준을 그대로 채택하지 않습니다. 망분리, 전자금융 규제와의 정합성을 함께 짚어야 합니다. 자세한 내용은 [한국 노트의 규제 페이지](/korea/regulation/)와 연결합니다.
