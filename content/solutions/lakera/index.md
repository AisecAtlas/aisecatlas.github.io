...
핵심 개념은 **런타임 가드레일**이다. LLM의 입력, 출력, RAG, MCP 흐름을 실시간으로 검사해 프롬프트 공격, 데이터 유출, 콘텐츠 위반을 차단한다. 배포 전 단계에서는 **AI 레드티밍**(Lakera Red)으로 가드레일 우회를 능동 시험한다. 차별점은 무료 AI 보안 테스트 게임 **Gandalf**에서 수집한 8천만 개 이상 공격 데이터 포인트로 공격 패턴을 학습한다는 점이다.

> **왜 중요한가.** 프롬프트 인젝션은 LLM 앱의 1순위 위협이다. Lakera는 배포 전 레드티밍과 런타임 가드레일을 한 벤더로 묶어 입력부터 출력까지 단일 흐름으로 방어한다.
...
Lakera는 LLM 런타임 가드레일 전문 업체로, 2025년 4분기 마감 예정으로 Check Point가 인수를 발표했다([Check Point](https://www.checkpoint.com/press-releases/check-point-acquires-lakera-to-deliver-end-to-end-ai-security-for-enterprises/)). 인수 금액은 비공개이나 약 **3억 달러**로 추정된다([Calcalist](https://www.calcalistech.com/ctechnews/article/rj5bc1vige)). 취리히 본사는 Check Point의 AI 보안 센터 기반이 된다.

경쟁 구도에서 Lakera는 런타임 프롬프트 가드레일 진영의 전문 주자다. 모델 공급망과 스캐닝 중심은 [Protect AI](/solutions/protect-ai/), 런타임과 공급망을 한 플랫폼으로 묶는 통합형은 [HiddenLayer](/solutions/hiddenlayer/)를 함께 검토하는 경우가 많다. 오픈소스로 직접 구성하려는 팀은 [NVIDIA NeMo Guardrails](/solutions/nemo-guardrails/)를 대안으로 본다. 최근 동향으로 Check Point 인수에 따라 기존 Check Point 보안 스택과의 통합이 진행 중이다([Check Point Blog](https://blog.checkpoint.com/security/check-point-to-acquire-lakera-redefining-security-for-the-ai-era/)).
...
**망분리.** 실시간 검사 SaaS형이다. 한국 공공과 금융의 망분리 환경 적용 가능성은 별도 검토가 필요하며, 한국어 프롬프트 공격과 우회 패턴의 탐지 정확도는 PoC에서 자체 데이터로 검증한다.
