...
Microsoft가 제공하는 클라우드 신원 플랫폼이다. 구 명칭은 Azure Active Directory로, 2023년 7월 11일 Entra ID 개명이 발표되고 8월 15일부터 전환이 시작됐다.
...
> **Conditional Access = Entra의 정책 결정점(PDP).** 사용자, 기기 컴플라이언스, 위험 점수, 위치, 앱 신호를 입력받아 grant control(MFA, 규정 준수 기기 요구)과 session control(앱 강제, 토큰 수명)로 집행한다.
...
위험 기반 조건부 접근, PIM 등 핵심 보안 기능을 본격적으로 쓰려면 P2나 Suite가 필요하다.

## 시장 위치

Entra ID는 Microsoft 365와 Azure를 사용하는 조직에서 사실상 표준 신원 플랫폼이다. M365 라이선스에 기본 신원 기능이 동봉돼 추가 비용 없이 시작할 수 있고, P1, P2, Suite로 올라가며 조건부 접근, Risk Engine, PIM, Private Access, CIEM이 단계적으로 추가되는 구조다(제3자 시장 점유율 수치는 본 페이지 출처상 미검증).

99.99% SLA는 Premium P1/P2 한정(인증 가용성 기준)이며 Free 에디션은 SLA가 없다. 구체 인증 처리량 공개 수치는 확인이 제한된다. 경쟁 구도에서 SaaS 카탈로그 통합 위주는 Okta를, 통제와 커스터마이징 우선은 Ping Identity를, 외부 고객 인증 위주는 Okta Auth0를 함께 검토하는 경우가 많다. 강점은 Microsoft 환경 통합이고, macOS/Linux/GCP/AWS 중심 환경에서는 그 우위가 줄어든다.
