...
Wiz는 흩어진 클라우드 보안 도구를 하나로 묶은 **에이전트리스 CNAPP**다. CSPM, CWPP, CIEM, DSPM, IaC 스캔을 단일 플랫폼에서 제공한다([출처](https://em360tech.com/tech-articles/solution-overview-what-wiz)).

핵심은 **Security Graph**다. 클라우드 전체를 속성 그래프로 보고, AWS, Azure, GCP, OCI, Kubernetes에 네이티브 API로 연결해 모든 노드와 엣지를 평가한다. 코드, 클라우드, 런타임의 위험을 상관 분석해 **악용 가능하고 도달 가능한 위험만** 골라내고, 그렇지 않은 경고는 억제해 알림 피로를 줄인다([출처](https://solideinfo.com/wiz-cloud-security/)).

> **왜 중요한가.** 멀티클라우드 환경에서 수많은 경고를 쏟아내는 대신, 실제로 악용 가능한 공격 경로만 선별해 우선순위를 잡아준다는 점이 차별점이다.
...
Wiz는 CNAPP 시장의 선도 사업자다. Fortune 100의 **45%**가 사용하고, **500만 개 이상 클라우드 워크로드**를 보호하며 하루 **2,300억 개 파일**을 스캔한다. **ARR 1억 달러를 18개월** 만에 달성했다([출처](https://cybersecuritytimes.com/google-completes-32b-acquisition-wiz-cloud-security-deal/)).

2026년 3월 Google이 **320억 달러** 전액 현금으로 인수를 완료했다([Google 발표](https://blog.google/inside-google/company-announcements/google-agreement-acquire-wiz/), [Alphabet 보도자료](https://www.sec.gov/Archives/edgar/data/1652044/000165204425000027/googexhibit99131825.htm)). 인수 이후 멀티클라우드 중립성 유지 여부가 시장의 관찰 대상이다.

경쟁 구도에서 Wiz는 에이전트리스 멀티클라우드 자세 관리와 공격 경로 가시화에 강점을 가진다. 네트워크 경계 ZTNA와 인라인 보안은 Zscaler, Palo Alto Prisma Access를, 엔드포인트 런타임 보호는 CrowdStrike Falcon을 함께 검토하는 경우가 많다.
...
**라이선스.** 엔터프라이즈 구독, 공개 정가 없음. 국내 파트너를 통한 도입이 일반적이다.

**데이터 리전.** 에이전트리스 SaaS로 클라우드 API에 연결한다. 스캔 데이터의 처리와 보관 위치는 도입 시 벤더에 직접 확인이 필요하다.

**CSAP.** Wiz의 한국 클라우드 보안인증(CSAP) 획득 여부는 공개 자료에서 확인되지 않는다. 모회사가 된 Google Cloud의 인증과는 별개이므로, 공공 도입 검토 시 Wiz 자체의 최신 인증 현황을 확인한다.

**망분리.** 멀티클라우드 SaaS 형태. 한국 공공과 금융의 망분리 환경 적용 가능성은 별도 검토가 필요하며, 국내 적용 사례는 공개 자료상 확인되지 않는다.
