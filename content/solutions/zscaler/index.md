...
Zscaler는 클라우드에서 동작하는 인라인 보안 플랫폼입니다.

핵심은 **Zero Trust Exchange**입니다. 전 세계 **150개 이상 데이터센터**에 분산된 SASE 아키텍처 기반 플랫폼으로, 사용자 트래픽을 가까운 데이터센터로 보내 검사하고 정책을 적용합니다.

기존 VPN은 사용자를 사내 네트워크에 들여보냅니다. Zscaler는 반대로 동작합니다. 네트워크를 사용자에게 확장하지 않고, 개별 디바이스와 개별 애플리케이션 사이에만 일대일 연결을 만듭니다. 이를 **segment of one**이라 부릅니다. 애플리케이션은 외부에 노출되지 않고 안에서 밖으로(inside-out) 연결을 맺습니다.

> **왜 중요한가.** 네트워크 중심 경계 모델에서 ID 우선, 애플리케이션 중심 모델로의 전환이 제로 트러스트의 핵심입니다. Zscaler는 이 전환을 제품 구조 자체로 강제합니다.

## 주요 모듈

| 모듈 | 역할 |
|---|---|
| Zscaler Internet Access (ZIA) | 인터넷 접근 보안. Cloud Firewall, Sandboxing, URL Filtering, DLP, CASB, Browser Isolation 통합 |
| Zscaler Private Access (ZPA) | 주력 ZTNA. 사설 애플리케이션에 대한 안전한 원격 접근, VPN 대체 |
| Zscaler Business to Business (ZB2B) | 파트너 연결. 앱을 인터넷에 노출하지 않는 서비스 개시형 ZTNA |
| Zscaler Cloud Protection (ZCP) | 워크로드 보안. Workload Posture, Private Access, Communications, Segmentation |
| Zscaler Digital Experience (ZDX) | 사용자와 앱 성능의 종단 간 가시성과 문제 진단 |
| Zscaler Deception (ZDT) | 기만 기술. 디코이로 능동 위협 탐지, lateral movement 차단 |

ZPA는 이 사이트의 평가 대상인 Networks 영역(ZTNA)에 직접 대응합니다.
...
| ID | 항목 | 점수 | 근거 |
|---|---|:-:|---|
| A-1 | 통합 관리 | 2/2 | Zero Trust Exchange 단일 플랫폼에 ZIA, ZPA, ZDX 등 모듈 통합 |
| A-2 | 가용성 | 2/2 | 150개 이상 글로벌 데이터센터 분산, 인라인 클라우드 구조 |
| A-3 | 성능 | 2/2 | 사용자 인근 데이터센터에서 검사, 글로벌 분산 처리 |
| A-4 | 운영성 | 1/2 | 모듈이 많아 초기 설계 학습 곡선이 있음. 한국어 지원 범위는 공개 자료상 확인 제한 |
...
| ZTA-NET-ZTNA-03 | 침해 가정 | 2/2 | 앱 비노출, Zero Trust Segmentation과 Deception으로 lateral movement 차단 |
| ZTA-NET-ZTNA-04 | 지속 검증 | 1/2 | 컨텍스트 기반 정책과 ZDX 모니터링은 있으나 지속 재검증의 세부 근거는 제한 |
| ZTA-NET-ZTNA-05 | 컨텍스트 정책 | 2/2 | 사용자, 디바이스, 앱 컨텍스트 기반 접근 정책 |
| ZTA-NET-ZTNA-06 | 전 구간 암호화 | 2/2 | TLS/SSL 검사와 암호화 트래픽 처리 |
| ZTA-NET-ZTNA-07 | 가시성 | 2/2 | ZDX로 사용자와 앱 성능의 종단 간 가시성 제공 |
...
- VPN을 ZTNA로 대체하려는 조직
- 다수 지점과 원격 사용자가 클라우드와 사내 앱에 동시에 접근하는 환경
- 인터넷 접근 보안(ZIA)과 사설 앱 접근(ZPA)을 한 플랫폼으로 묶고 싶을 때
...
**데이터 리전.** Zscaler는 글로벌 150개 이상 데이터센터를 운영합니다. 한국(서울) 데이터센터의 정확한 운영과 인증 상태는 [Zscaler 데이터센터 맵](https://trust.zscaler.com/data-center-map)에서 별도 확인이 필요합니다.
...
**망분리.** Zscaler는 클라우드 SaaS형 인라인 보안 플랫폼입니다. 한국 공공과 금융의 망분리 환경 적용 가능성은 별도 검토가 필요하며, 이번 검토에서 국내 적용 사례는 확인하지 못했습니다.
...
- 2025년 Zero Trust Segmentation, SAP RISE 네이티브 통합 등 플랫폼 확장 지속

## 한계

- 모듈 구성이 많아 초기 설계와 정책 튜닝에 학습 비용
...
- [CrowdStrike Falcon](/solutions/crowdstrike-falcon/)
- [Okta](/solutions/okta/)
- [Microsoft Entra ID](/solutions/entra-id/)
