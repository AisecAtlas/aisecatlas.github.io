...
Harmonic Security는 직원의 생성형 AI 사용을 발견하고, 민감 데이터가 AI 도구로 새는 것을 막는 데이터 보호 플랫폼이다. 제품명은 **Harmonic Protect**. 설계의 중심은 **브라우저 확장**이다.

다른 Shadow AI 통제가 네트워크 프록시에 의존하는 것과 달리, Harmonic은 **브라우저 확장(Chrome, Edge, Firefox, Safari)을 주 센서**로 쓰고, 워크스테이션의 데스크톱 AI 앱과 에이전트를 위해 **엔드포인트 에이전트**를 더한다. 프롬프트는 DOM에서 직접 읽히므로 TLS 암호화나 인증서 핀닝과 무관하게 내용을 본다. 캡처 메커니즘의 일반 비교는 [AI 사용 가시화](/ai/ai-usage-visibility/)가 정리한다.

> **왜 브라우저인가.** 직원의 미승인 AI 사용은 대부분 웹 브라우저에서 일어난다. 프록시는 인증서 핀닝 앱과 SaaS 내장 AI에서 사각이 생기지만, DOM 계층 캡처는 렌더된 화면을 읽어 개인 계정 사용까지 포착한다.
...
Harmonic Security는 Shadow AI 발견에 특화한 순수 플레이다. 2023년 설립됐고 창업자는 Digital Shadows를 만든 Alastair Paterson과 Bryan Woolgar-O'Neil이다.

투자는 2023년 10월 시드 **$7M**(Ten Eleven Ventures), 2024년 10월 2일 시리즈 A **$17.5M**(Next47 주도)로 누적 **$24.5M 이상**이다([Business Wire](https://www.businesswire.com/news/home/20241002920021/en/)).

경쟁 구도에서 Harmonic은 단말 기반(브라우저, 엔드포인트) 진영이다. 네트워크 인라인 진영의 [WitnessAI](/solutions/witnessai/), 발견과 강한 DLP를 묶은 Nightfall AI, 데이터 리니지 기반의 Cyberhaven과 함께 검토되는 경우가 많다. 이미 SASE/SSE(Zscaler, Netskope)를 쓰는 조직은 플랫폼 네이티브 GenAI 모듈과 중복 여부를 따진다.
...
**라이선스.** 공개 정가 없음. 비상장 초기 단계로 국내 도입 경로는 직접 문의가 필요하다.

**데이터 리전.** 브라우저, 엔드포인트 텔레메트리의 처리, 보관 위치를 확인한다. 한국 데이터 리전 보장은 공개 자료에서 확인되지 않는다.

**CSAP.** 한국 클라우드 보안인증 획득 여부는 공개 자료에서 확인되지 않는다. 단말 기반 캡처는 클라우드 경유 없이 적용 가능하나, 백엔드 분석의 처리 위치를 확인한다.

**개인정보보호법.** 프롬프트, 붙여넣기 내용을 단말에서 읽으므로, 직원 모니터링 범위와 동의, 처리 데이터 최소화를 사전 검토한다.

**망분리.** 단말 기반이라 인라인 클라우드 경유 없이 적용 가능하나, 백엔드 SLM 분석의 위치와 통신 경로를 확인한다.
