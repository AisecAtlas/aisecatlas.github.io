...
WitnessAI는 직원과 AI(사람과 에이전트)의 상호작용을 가시화하고 정책을 거는 "Secure AI Enablement" 플랫폼이다. 단말에 아무것도 설치하지 않는 **에이전트리스 네트워크 인라인** 방식이 특징이다.

사용자와 LLM 사이의 인프라 계층에 인라인으로 끼어 밀리초 지연으로 모든 AI 상호작용을 본다. 단말 설치가 없어 배포가 단순하고, 데스크톱 AI 앱처럼 엔드포인트 도구가 약한 영역도 커버한다. 단, 네트워크 인라인이므로 콘텐츠 가시성은 TLS 처리 구조에 의존한다 — 캡처 메커니즘 일반 비교는 [AI 사용 가시화](/ai/ai-usage-visibility/)가 정리한다.

> **차별점: 의도 분류.** WitnessAI는 에이전트의 행동이 프롬프트의 의도와 일치하는지 검사하는 의도 분류(intention classification) 모델을 둔다. 단순 입출력 차단을 넘어 에이전트의 일탈 행동을 본다.
...
WitnessAI는 단말 설치 없는 네트워크 인라인 AI 거버넌스 진영의 대표 주자다. 2023년 설립됐다.

투자는 2024년 5월 시리즈 A **$27.5M**(GV와 Ballistic Ventures 공동 주도), 2026년 1월 13일 전략 투자 **$58M**(Sound Ventures 주도, Fin Capital, Qualcomm Ventures, Samsung Ventures, Forgepoint 참여)로 누적 약 **$85.5M**이다([PR Newswire](https://www.prnewswire.com/news-releases/witnessai-raises-58-million-for-global-expansion-and-announces-new-ways-to-secure-ai-agents-302659319.html)).

경쟁 구도에서 WitnessAI는 단말 기반의 [Harmonic Security](/solutions/harmonic-security/), Nightfall AI와 대비된다. 단말 설치 제약이 있는 환경에서 인라인 방식의 장점이 크다. 이미 SASE/SSE(Zscaler AI Access Security, Netskope, Palo Alto AI Access Security)를 쓰는 조직은 플랫폼 네이티브 GenAI 모듈과 역할이 겹치는지 따진다.
...
**라이선스.** 공개 정가 없음. 비상장 단계로 국내 도입 경로는 직접 문의가 필요하다.

**데이터 리전.** 인라인 처리 위치와 텔레메트리 보관 리전을 확인한다. 2026-01 투자에 Samsung Ventures, Qualcomm Ventures가 참여해 국내 진출 가능성은 있으나 공식 한국 리전, 총판은 미발표다.

**CSAP.** 한국 클라우드 보안인증 획득 여부는 공개 자료에서 확인되지 않는다. 클라우드 인라인 경유는 공공 도입 시 인증과 리전 확인이 선행된다.

**개인정보보호법.** 인라인으로 프롬프트와 응답을 처리하므로, 직원 AI 사용 모니터링 범위와 동의, 처리 최소화를 사전 검토한다.

**망분리.** 클라우드 인라인 방식이라 망분리 환경 적용 가능성은 별도 검토가 필요하며, 국내 적용 사례는 공개 자료상 확인되지 않는다.
