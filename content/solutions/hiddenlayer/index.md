...
핵심 개념은 **AIDR**(AI Detection and Response, 런타임 방어)다. AIDR의 **결정적 분류기가 모델 추론 경로 밖에 위치**해, 프롬프트 인젝션, 탈옥, 간접 공격을 탐지하고 데이터 유출과 악성 출력을 차단한다([HiddenLayer AIDR](https://hiddenlayer.com/aidr/)). 분류기가 경로 밖이라 모델 응답 지연에 영향이 적다. 여기에 모델 스캐닝으로 공급망 위험을 점검하고, **AIBOM과 Model Genealogy**로 AI 자산 인벤토리와 출처를 추적한다.

> **왜 중요한가.** AI 보안은 런타임, 공급망, 자산 가시성, 레드티밍이 따로 도구화되기 쉽다. HiddenLayer는 이를 한 플랫폼으로 통합해 AI 자산 전반을 가시화한다.
...
HiddenLayer는 AI 보안만 전담하는 독립 업체다. 2022년 설립해 총 **5,600만 달러**를 유치했고, 투자자에 M12(Microsoft), IBM Ventures, Capital One Ventures가 있다([HiddenLayer](https://www.hiddenlayer.com/news/hiddenlayer-raises-50m-in-series-a-funding-to-safeguard-ai)). 다수 경쟁사가 대형 보안 벤더에 인수된 가운데 독립 운영을 유지하는 점이 특징이다.

경쟁 구도에서 HiddenLayer는 런타임과 공급망을 한 플랫폼으로 통합하는 진영의 주자다. 모델 스캐닝과 공급망 영역에서 [Protect AI](/solutions/protect-ai/)와 경합하고, 런타임 프롬프트 가드레일 특화는 [Lakera](/solutions/lakera/)가 더 깊다. 최근 동향으로 2025년 4월 22일 가시성과 관측성을 강화한 **AISec Platform 2.0**을 발표했다([PR Newswire](https://www.prnewswire.com/news-releases/hiddenlayer-unveils-aisec-platform-2-0-to-deliver-unmatched-context-visibility-and-observability-for-enterprise-ai-security-302434498.html)).
...
**망분리.** 런타임 검사와 자세 관리 SaaS형이다. 한국 공공과 금융의 망분리 환경 적용 가능성은 별도 검토가 필요하며, 한국어 프롬프트 공격과 우회 패턴의 탐지 정확도는 PoC에서 자체 데이터로 검증한다.
