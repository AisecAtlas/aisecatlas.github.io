...
> **이 페이지의 결론**: 한국 공공 부문은 19년간 유지한 획일적 물리 망분리를 폐기하고, 정보 중요도에 따라 보안을 차등 적용하는 **국가망보안체계(N²SF)**로 전환 중입니다. N²SF 정식 가이드라인에 제로트러스트 원칙이 명시적으로 반영되면서, "망분리라서 클라우드와 AI를 못 쓴다"는 제약이 풀리는 방향으로 정책이 움직이고 있습니다.
...
> **핵심.** 망분리는 "안과 밖을 물리적으로 끊는다"는 경계 모델입니다. N²SF는 "정보 등급에 따라 신뢰를 차등 검증한다"는 제로트러스트 모델로 옮겨가는 첫 국가 단위 정책입니다.
...
본 페이지는 공개 자료를 근거로 합니다. 정책 전환이 진행 중인 영역이며, 가이드라인 세부 내용은 후속 개정으로 바뀔 수 있습니다. 권위 있는 해석이 필요하면 KISA와 국정원의 원문 가이드라인을 확인하세요.
