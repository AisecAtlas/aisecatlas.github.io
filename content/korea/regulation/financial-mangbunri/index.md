...
규제가 무엇을 하지 말라는 행위 나열에서 위험을 스스로 평가하고 통제하라는 원칙 기반으로 바뀝니다. 자율보안과 제로트러스트가 그 빈자리를 채웁니다.
...
개정 핵심은 개인신용정보를 저장하지 않고 전자금융거래 중요성이 낮은 **개발, 테스트 서버의 물리 망분리 완화**, 그리고 연구개발 목적이나 비중요 업무에서 금융회사가 **자체 위험평가를 거쳐 망분리 예외**를 적용하도록 허용한 점입니다([달콤한 보안망고](https://securitymango.com/financialit_0005/)). 고객관리(CRM), 보안관리 업무 등에서 SaaS 이용 범위도 확대되었습니다.
