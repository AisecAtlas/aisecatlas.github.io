...
> **자동화 자체는 새로운 것이 아닙니다.** 다시 화두가 된 이유는 LLM 기반 추론이 가용해지면서, 정해진 일뿐 아니라 **정해지지 않은 일도 안전하게 수행**할 수 있게 됐기 때문입니다.
...
> **본질의 차이.** SOAR가 손이 빠른 직원이라면, Agentic SOC는 **판단할 줄 아는 주니어 분석가**입니다. 가드레일 안에서 자율적으로 일합니다.
...
자세한 항목별 비교는 [Security Copilots 비교](security-copilots/)에서 확인하세요.

## 자동화 도입 시 주의할 점

기술 도입 자체보다 거버넌스 설계가 더 어렵습니다. 다섯 가지를 미리 답하지 못하면 도입 후 문제가 됩니다.
...
**금융권.** 자율 대응은 금감원 보안성 심의에서 면밀히 검토됩니다. 감사 추적과 인간 승인 흐름을 처음부터 명확히 설계해야 합니다.

**공공 부문.** CSAP 인증과 망분리가 적용 범위를 좁힙니다. 사내 호스팅 SOAR와 자체 가드레일 조합이 현실적입니다.
